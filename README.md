# RDK2 QSPI PSRAM Dynamic Allocation Example

Rutronik Development Kit Programmable System-on-Chip CY8C6245AZI-S3D72 "QSPI PSRAM Dynamic Allocation" Example. 

This example demonstrates how to configure and use the PSRAM APS6404L-3SQR-ZR with standard dynamic memory allocation functions, such as malloc().
...
The PSRAM is configured to work in XIP (memory-mapped) mode before any dynamic allocation operation can take place. The limited range of random numbers is generated to allocate a variable amount of space in the external memory on every cycle. The allocated memory can also be tested by defining it. After the allocation and/or testing is complete, the allocated memory is freed. This cycle is infinite in a given example. 

The randomised memory allocation size can be disabled by setting ALLOC_FIXED_SIZE to the fixed size (up to 8MB). 

The custom **_sbrk()** function needs to be provided for standard memory allocators, such as malloc() or calloc(). The mentioned function is provided in the example project "sbrk.c" file.  Its purpose is to expand the memory to the external PSRAM memory locations defined by `PSRAM_START_ADDR` and `PSRAM_END_ADDR` definitions in the "sbrk.c" file. 

The results can be seen in the terminal output from the KitProg3 UART and in the user LEDs.
